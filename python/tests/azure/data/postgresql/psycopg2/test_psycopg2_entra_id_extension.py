...
functionality for PostgreSQL connections using psycopg2. These tests serve as
...
Usage:
    ...
    pytest test_psycopg2_entra_id_extension.py::TestConnectWithEntra
...
from azurepg_entra.psycopg2 import (
    connect_with_entra,
...
class TestGetEntraConninfo:
    ...
    def test_get_entra_conninfo_fallback_to_management_scope(self):
        ...
        with patch('azurepg_entra.psycopg2.psycopg2_entra_id_extension.get_entra_token') as mock_get_token:
    ...
    async def test_get_entra_conninfo_async_with_credential(self):
        ...
        with patch('azurepg_entra.psycopg2.psycopg2_entra_id_extension.get_entra_token_async', return_value=token):
    ...
    async def test_get_entra_conninfo_async_fallback_to_management_scope(self):
        ...
        with patch('azurepg_entra.psycopg2.psycopg2_entra_id_extension.get_entra_token_async') as mock_get_token:
            mock_get_token.side_effect = [db_token, mgmt_token]
...
class TestConnectWithEntra:
    """
    Tests for the connect_with_entra function.
    
    This function handles Azure Entra ID authentication for psycopg2 connections.
    Tests validate:
    - Proper credential handling and validation
    - Fallback to standard authentication when credentials exist
    - Integration with the underlying psycopg2.connect logic
    """
    ...
    def test_connect_with_user_and_password(self):
        ...
        with patch('psycopg2.connect') as mock_connect:
            ...
            result = connect_with_entra(**kwargs)
    ...
    def test_connect_without_user_password_with_credential(self):
        ...
        with patch('azurepg_entra.psycopg2.psycopg2_entra_id_extension.get_entra_conninfo', return_value=expected_conninfo) as mock_get_conninfo:
            with patch('psycopg2.connect') as mock_connect:
                mock_connection = Mock()
                mock_connect.return_value = mock_connection
                
                result = connect_with_entra(credential=mock_credential, **kwargs)
    ...
    def test_connect_invalid_credential_type(self):
        ...
        with pytest.raises(ValueError, match="credential must be a TokenCredential for synchronous connections"):
            connect_with_entra(credential=invalid_credential, **kwargs)


class TestConnectWithEntraAsync:
    """
    Tests for the connect_with_entra_async function.
    
    This function handles Azure Entra ID authentication for aiopg connections.
    Tests mirror the synchronous tests to ensure consistent behavior between 
    sync/async usage patterns.
    """
    ...
    async def test_connect_with_user_and_password(self):
        ...
        with patch('aiopg.connect', new_callable=AsyncMock) as mock_connect:
            mock_connection = Mock()
            mock_connect.return_value = mock_connection
            
            result = await connect_with_entra_async(**kwargs)
    ...
    async def test_connect_without_user_password_with_credential(self):
        ...
        with patch('azurepg_entra.psycopg2.psycopg2_entra_id_extension.get_entra_conninfo_async', return_value=expected_conninfo) as mock_get_conninfo:
            with patch('aiopg.connect', new_callable=AsyncMock) as mock_connect:
                mock_connection = Mock()
                mock_connect.return_value = mock_connection
                
                result = await connect_with_entra_async(credential=mock_credential, **kwargs)
    ...
    async def test_connect_invalid_credential_type(self):
        ...
        with pytest.raises(ValueError, match="credential must be an AsyncTokenCredential for async connections"):
            await connect_with_entra_async(credential=invalid_credential, **kwargs)
...
4. Connection Functions (TestConnectWithEntra/TestConnectWithEntraAsync):
